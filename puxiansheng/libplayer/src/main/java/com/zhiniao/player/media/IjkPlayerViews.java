package com.zhiniao.player.media;
...
import com.zhiniao.libplayer.R;
import com.zhiniao.player.danmaku.BaseDanmakuConverter;
import com.zhiniao.player.danmaku.BiliDanmukuParser;
import com.zhiniao.player.danmaku.OnDanmakuListener;
import com.zhiniao.player.utils.AnimHelper;
import com.zhiniao.player.utils.MotionEventUtils;
import com.zhiniao.player.utils.NavUtils;
import com.zhiniao.player.utils.NetWorkUtils;
import com.zhiniao.player.utils.SDCardUtils;
import com.zhiniao.player.utils.SoftInputUtils;
import com.zhiniao.player.utils.StringUtils;
import com.zhiniao.player.utils.WindowUtils;
import com.zhiniao.player.widgets.MarqueeTextView;
import com.zhiniao.player.widgets.ShareDialog;
 ...
 */
public class IjkPlayerViews extends FrameLayout implements View.OnClickListener, IjkVideoView.Videoscale {
    // 进度条最大值
...
//                    Log.d("ijkplayer"," mVideoView.isPlaying()");
    ...
    public IjkPlayerViews(Context context) {
        this(context, null);
    }

    public IjkPlayerViews(Context context, AttributeSet attrs) {
        super(context, attrs);
        _initViews(context);
    }
...
//        mAspectRatioOptions.setVisibility(View.GONE);
    }
...
//        mVideoView.getMediaPlayer().setOnVideoSizeChangedListener(new IMediaPlayer.OnVideoSizeChangedListener() {
//            @Override
//            public void onVideoSizeChanged(IMediaPlayer iMediaPlayer, int i, int i1, int i2, int i3) {
//                Log.i("test", "第一个参数" + i + "第二个参数" + i1 + "第三个参数" + i2 + "第四个参数" + i3);
//            }
//        });
     ...
     * @return
     */
    OnPlayViewClick onPlayViewClick;

    String type = "";

    public IjkPlayerViews init(OnPlayViewClick onPlayViewClick, String type) {
        _initMediaPlayer();
        this.onPlayViewClick = onPlayViewClick;
        this.type = type;
        return this;
    }


    public IjkPlayerViews init(OnPlayViewClick onPlayViewClick) {
        _initMediaPlayer();
        this.onPlayViewClick = onPlayViewClick;
        return this;
    }
     ...
     * @return
     */
    public IjkPlayerViews switchVideoPath(String url) {
     ...
     * @return
     */
    public IjkPlayerViews switchVideoPath(Uri uri) {
     ...
     * @return
     */
    public IjkPlayerViews setVideoPath(String url) {
     ...
     * @return setVideoURIs
     */
    public IjkPlayerViews setVideoPath(Uri uri) {
    ...
    public IjkPlayerViews setVideoPaths(Uri uri) {
        mVideoView.setVideoURIs(uri, this);
        if (mCurPosition != INVALID_VALUE) {
            seekTo(mCurPosition);
            mCurPosition = INVALID_VALUE;
        } else {
     ...
     * @param title
     */
    public IjkPlayerViews setTitle(String title) {
     ...
     * 设置只显示全屏状态
     */
    public IjkPlayerViews alwaysFullScreen() {
...
//        mPlayerThumb.setVisibility(View.GONE);
     ...
     * 使能视频翻转
     */
    public IjkPlayerViews enableOrientation() {
    ...
    private void _toggleFullScreen() {
        if (WindowUtils.getScreenOrientation(mAttachActivity) == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
            mAttachActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
            Log.i("test", "竖屏");
            if (null != onPlayViewClick)
                onPlayViewClick.setOnClick(0);
        } else {
            mAttachActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
            Log.i("test", "全屏");
            if (null != onPlayViewClick)
                onPlayViewClick.setOnClick(1);
        }
    }
...
//        mVideoView.setOnCompletionListener(l);
    ...
    public interface OnPlayViewClick {
        void setOnClick(int click);
    }
     ...
     */
    public IjkPlayerViews setDanmakuListener(OnDanmakuListener danmakuListener) {
    ...
    private void _initMediaQuality() {
        ...
        mLvMediaQuality.setAdapter(mQualityAdapter);//mCurSelectQuality
     ...
     * @return
     */
    public IjkPlayerViews switchVideoSource(String mediaSmooth, String mediaMedium, String mediaHigh, String mediaSuper, String mediaBd) {
        reset();
     ...
     * @param mediaBd     1080P
     */
    public IjkPlayerViews setVideoSource(String mediaSmooth, String mediaMedium, String mediaHigh, String mediaSuper, String mediaBd) {
        boolean isSelect = true;
        mQualityData = new ArrayList<>();
        if (mediaSmooth != null) {
            mVideoSource.put(MEDIA_QUALITY_SMOOTH, mediaSmooth);
            mQualityData.add(new MediaQualityInfo(IjkPlayerView.MEDIA_QUALITY_SMOOTH, mMediaQualityDesc[MEDIA_QUALITY_SMOOTH], isSelect));
            mCurSelectQuality = MEDIA_QUALITY_SMOOTH;
            isSelect = false;
        }
        if (mediaMedium != null) {
            mVideoSource.put(MEDIA_QUALITY_MEDIUM, mediaMedium);
            mQualityData.add(new MediaQualityInfo(IjkPlayerView.MEDIA_QUALITY_MEDIUM, mMediaQualityDesc[MEDIA_QUALITY_MEDIUM], isSelect));
            if (isSelect) {
        ...
        if (mediaHigh != null) {
            ...
            mQualityData.add(new MediaQualityInfo(IjkPlayerView.MEDIA_QUALITY_HIGH, mMediaQualityDesc[MEDIA_QUALITY_HIGH], isSelect));
            if (isSelect) {
        ...
        if (mediaSuper != null) {
            ...
            mQualityData.add(new MediaQualityInfo(IjkPlayerView.MEDIA_QUALITY_SUPER, mMediaQualityDesc[MEDIA_QUALITY_SUPER], isSelect));
            if (isSelect) {
        ...
        if (mediaBd != null) {
            ...
            mQualityData.add(new MediaQualityInfo(IjkPlayerView.MEDIA_QUALITY_BD, mMediaQualityDesc[MEDIA_QUALITY_BD], isSelect));
            if (isSelect) {
     ...
     * @return
     */
    public IjkPlayerViews setMediaQuality(@MediaQuality int quality) {
     ...
     */
    public IjkPlayerViews setSkipTip(int targetPosition) {
    ...
    private void _showSkipTip() {
        if (mSkipPosition != INVALID_VALUE && mLlSkipLayout.getVisibility() == GONE) {
            ...
            mTvSkipTime.setText(StringUtils.generateTime(mSkipPosition));
...
//            mDanmakuContext.setDanmakuSync(new VideoDanmakuSync(this));
     ...
     * @return
     */
    public IjkPlayerViews enableDanmaku() {
     ...
     * @return
     */
    public IjkPlayerViews enableDanmaku(boolean isEnable) {
     ...
     * @return
     */
    public IjkPlayerViews setDanmakuSource(InputStream stream) {
        if (stream == null) {
            return this;
        }
        if (!mIsEnableDanmaku) {
     ...
     * @return
     */
    public IjkPlayerViews setDanmakuSource(String uri) {
     ...
     * @return
     */
    public IjkPlayerViews setDanmakuCustomParser(BaseDanmakuParser parser, ILoader loader, BaseDanmakuConverter converter) {
     ...
     * @return
     */
    public IjkPlayerViews showOrHideDanmaku(boolean isShow) {
        if (isShow) {
     ...
     * @return
     */
    public IjkPlayerViews setDialogClickListener(ShareDialog.OnDialogClickListener dialogClickListener) {
     ...
     * @param path
     */
    public IjkPlayerViews setSaveDir(String path) {
        _createSaveDir(path);
        return this;
    }
    ...
    @Override
    public void setRatio(boolean falg) {
        this.ratio = falg;
    }
}

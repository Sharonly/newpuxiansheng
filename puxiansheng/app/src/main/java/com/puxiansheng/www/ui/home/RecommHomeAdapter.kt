...
class RecommHomeAdapter(var context: Context, var dataList: ArrayList<OrderDetailObject>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == 1) {
            val view = LayoutInflater.from(context)
                .inflate(R.layout.fragment_home_out_item, parent, false)
            return RecommendTransferOutViewHolder(view)
        } else if (viewType == 2) {
            val view = LayoutInflater.from(context)
                .inflate(R.layout.fragment_home_in_item, parent, false)
...
//            }
            ...
            var rentStr = shopInfo.formattedRent
            if (rentStr!= null && rentStr.length > 4) {
                holder.shopRent.text = rentStr.substring(0, rentStr.length - 3)
                holder.unit.text = rentStr.substring(rentStr.length - 3)
            } else {
                holder.shopRent.text = shopInfo.formattedRent
            }
//            var htmlStr3 = "<font  size='15px'>${rentStr.substring(0,rentStr.length - 3)}</font>" +
//                    "<font  size='10px'>${rentStr.substring(rentStr.length - 3)}</font>"
//            holder.shopRent.text = Html.fromHtml(htmlStr3)
            ...
            var shopInfo = dataList[position]
            holder.shopTitle.text = shopInfo.title
            holder.shopIndustry.text = "行业:" + shopInfo.formattedFinalIndustry
            holder.shopSize.text = "面积:" + shopInfo.view_acreage_un_prefix
            holder.shopRent.text = "租金:" + shopInfo.view_rent_un_prefix
            holder.shopArea.text = "区域:" + shopInfo.show_area
            holder.shopData.text = shopInfo.day_time
            holder.root.setOnClickListener {
                if (Utils.isFastClick()) {
    ...
    class RecommendTransferOutViewHolder(containerView: View) :
        ...
        val unit = containerView.findViewById<TextView>(R.id.unit)
